...
function newRemote(type, data)
    ...
    SendInfoToTurtleServer(BlockCode(genScript(log.Remote, log.args)))
    layoutOrderNum -= 1
...
newButton("Auto run Code",function()
    LoopingScript = not LoopingScript
      while wait() do -- 1
        if LoopingScript == false then break end
 local Remote = selected and selected.Remote
        if Remote then -- 2
            -- TextLabel.Text = "Executing..."
            -- xpcall(function()
